...
Making a basic request with method POST
You can set the HTTP METHOD with -X, example for make a POST request

curl -X POST http://localhost:3000
Making a basic request with method POST passing Header.
CURL does not pass the JSON header by default, to pass a body content to our API, you will need to pass the heder, see how using the -H

curl -X POST -H "Content-Type: application/json" http://localhost:3000
Making a basic complete post request with body data.
You can pass json in body data request using the -d.

curl -X POST -H "Content-Type: application/json" -d '{"key":"value"}' http://localhost:3000/product
Creating a product.
In your terminal, run some times, change the name and price. Create some products.

curl -X POST -H "Content-Type: application/json" -d '{"name":"product 01", "price": "399"}' http://localhost:3000/product

Updating a product
In your terminal, run passing /id. Lets update the product with id 1.

curl -X PUT -H "Content-Type: application/json" -d '{"name":"product 02 updated", "price": "7500"}' http://localhost:3000/product/1
